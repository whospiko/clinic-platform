...
export class AutoMigration1788343889159 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX \`idx_waitlist_entries_status_doctor_range\` ON \`waitlist_entries\``,
    );
    await queryRunner.query(
      `DROP INDEX \`idx_waitlist_entries_patient_status\` ON \`waitlist_entries\``,
    );
    await queryRunner.query(
      `DROP INDEX \`idx_waitlist_entries_offer_expiry\` ON \`waitlist_entries\``,
    );
