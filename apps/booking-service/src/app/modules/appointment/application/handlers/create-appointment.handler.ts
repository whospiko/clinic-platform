  ...
  implements ICommandHandler<CreateAppointmentCommand, AppointmentResponseDto>
{
  ...
  async execute(
    ...
    Logger.debug(
      `Creating appointment for patient ${command.patientId} with doctor ${command.doctorId} starting at ${command.startAt.toISOString()}`,
    );
    ...
    Logger.debug(
      `Saving appointment ${appointment.getAppointmentNo()} for patient ${appointment.getPatientId()} with doctor ${appointment.getDoctorId()} starting at ${appointment.getStartAt().toISOString()}`,
    );
  ...
  private async resolveEndAt(command: CreateAppointmentCommand): Promise<Date> {
    ...
    if (command.treatmentId) {
      const treatment = await this.treatmentReader.getTreatment(
        command.treatmentId,
      );
      ...
      return new Date(
        command.startAt.getTime() + treatment.durationMinutes * 60_000,
      );
    }
