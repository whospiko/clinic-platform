...
export class WaitlistEntryAggregate {
  ...
  isActive(): boolean {
    return [WaitlistEntryStatus.WAITING, WaitlistEntryStatus.OFFERED].includes(
      this.props.status,
    );
  }
  ...
  offerSlot(params: {
    ...
    if (offeredMinutes < this.props.requestedDurationMinutes) {
      throw new Error('Offered slot is shorter than requestedDurationMinutes');
    }
  ...
  acceptOffer(params: { appointmentId: string; now?: Date }): void {
    const now = params.now ?? new Date();
  ...
  cancel(params: { reason?: string | null; now?: Date }): void {
    const now = params.now ?? new Date();
