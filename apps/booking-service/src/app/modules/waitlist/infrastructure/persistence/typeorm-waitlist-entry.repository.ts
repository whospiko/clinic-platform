...
export class TypeOrmWaitlistEntryRepository implements WaitlistEntryRepository {
  ...
  async findActiveDuplicate(params: {
    ...
    const query = this.repository
      ...
      .andWhere('waitlist.status IN (:...statuses)', {
        statuses: [WaitlistEntryStatus.WAITING, WaitlistEntryStatus.OFFERED],
      })
      .andWhere(
        new Brackets((qb) => {
          qb.where('waitlist.preferredStartAt <= :preferredEndAt', {
            preferredEndAt: params.preferredEndAt,
          }).andWhere('waitlist.preferredEndAt >= :preferredStartAt', {
  ...
  async findCandidatesForSlot(
    ...
    const query = this.repository
      ...
      .andWhere('waitlist.requestedDurationMinutes <= :slotMinutes', {
        slotMinutes,
      })
      .andWhere(
        new Brackets((qb) => {
